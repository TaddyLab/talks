...
{\bf Semi-parametrics in treatment effect estimation}

\vskip .5cm
Say $\lambda = \ds{E}[z|z\geq u]$.

\vskip .25cm
Given treatment and control samples and the above SPB model, we can derive the mean and variance of $\gamma = \mu_1 - \mu_0$ up to $\ds{E}[\lambda_1 - \lambda_0]$ and $\mr{var}(\lambda_1 - \lambda_0)$.

\vskip .25cm
So we need MCMC, but just on the little bit of data in the tail.  

\vskip .1cm
{\gr And we have a new super simple Independence MH algorithm that proposes from a parametric bootstrap.}

\end{frame}

\begin{frame}

The I-MH algorithm takes a parametric bootstrap for the MAP of [$\xi,\sigma$], then estimates the density of this `proposal' with a kernel smoother and uses this to calculate acceptance probabilities.

\vskip .5cm
\includegraphics[width=1.62\textwidth]{graphs/tail-tid33203-cut2000}

\end{frame}

\begin{frame}

And we can use resulting posterior on $\gamma = \mu_1 - \mu_0$ to evaluate the accuracy of other common inference techniques.  

\vskip .5cm
For example, naive inference just looks at $\bar y_1 - \bar y_2$, while `capping' or winsorization replaces all values above a cap with that threshold.

\includegraphics[width=\textwidth]{graphs/winsorized-cut2000-tid33203}
